...
function deletePost(){
	...
	if (confirmed) {
		...
		formObj.method = "POST";
		formObj.action = "./notice_delete.do";
		...
		<div class="bodytext_area box_inner">			
			...
			<p class="btn_line txt_right">
				<c:if test="${ dto.id eq userId }">
					<a href="./notice_edit.do?num=${ dto.num }" class="btn_bbs">수정</a>
					<a onclick="deletePost();" class="btn_bbs">삭제</a>
				</c:if>
				
				
				<a href="./notice_list.do" class="btn_bbs">목록</a>
			</p>
			<ul class="near_list mt20">
				<li><h4 class="prev">다음글</h4>
					<c:choose>
						...
						<c:otherwise>
							<a href="./notice_view.do?num=${ nextDto.num }">${ nextDto.title }</a>
						</c:otherwise>
					</c:choose>
				</li>		
				<li><h4 class="next">이전글</h4>
					<c:choose>
						<c:when test="${ empty prevDto }">
							이전 글이 존재하지 않습니다.
						</c:when>
						<c:otherwise>
							<a href="./notice_view.do?num=${ prevDto.num }">${ prevDto.title }</a>
						</c:otherwise>
					</c:choose>
				</li>
			</ul>
